...
async def admin_toggle_product(sku: str, payload: dict, admin: dict = Depends(require_admin)):
    ...
    prod = await db.lolodrive_products.find_one({"sku": sku}, {"_id": 0, "name": 1})
    if not prod:
        raise HTTPException(status_code=404, detail="Produit introuvable")
    await db.lolodrive_products.update_one({"sku": sku}, {"$set": {"is_active": is_active}})
    from datetime import datetime as dt
    await db.product_toggle_logs.insert_one({
        "sku": sku, "name": prod.get("name"), "is_active": is_active,
        "by_email": admin.get("email"), "by_name": admin.get("contact_name") or admin.get("email"),
        "at": dt.utcnow()})
...
async def admin_products_tva(admin: dict = Depends(require_admin)):
    ...
    products = await db.lolodrive_products.find(
        ...
        {"_id": 0, "sku": 1, "name": 1, "category": 1, "subcategory": 1, "point_code": 1, "is_active": 1, "supplier": 1, "supplier_email": 1, "purchase_price_cents": 1,
