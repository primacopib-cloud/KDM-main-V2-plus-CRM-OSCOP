...
def build_restock_pdf(order: dict, point: dict) -> bytes:
    ...
    cols = [20 * mm, 95 * mm, 145 * mm, 168 * mm]
    c.setFont("Helvetica-Bold", 9)
    for x, label in zip(cols, ["Produit", "Fournisseur", "Qté cdée", "Qté reçue"]):
    ...
    for l in order.get("lines", []):
        c.drawString(cols[0], y, (l.get("name") or l["sku"])[:52])
        c.drawString(cols[1], y, (l.get("supplier") or "—")[:32])
        c.drawRightString(cols[2] + 12 * mm, y, str(l["qty"]))
        c.drawRightString(cols[3] + 14 * mm, y, str(got.get(l["sku"], "—")) if order.get("received_at") else "—")
        y -= 5.5 * mm
        if y < 25 * mm:
            c.showPage()
            c.setFont("Helvetica", 9)
