...
from vendor_models import (
    ...
    ProductSubmission, ProductUpdate, ProductDocument, COUNTRIES, clean_incoterms,
)
...
async def submit_product(vendor_id: str, data: ProductSubmission, request: Request):
    ...
    await consume_credits(vendor_id, "product_submission", f"Fiche produit {data.name}",
                          ...
                          territory=(data.available_zones[0] if data.available_zones else None))
    ...
    product = {
        ...
        # Incoterms par zone
        "incoterms": clean_incoterms(data.incoterms, data.available_zones),
...
async def update_product(vendor_id: str, product_id: str, data: ProductUpdate, request: Request):
    ...
    if "incoterms" in update_data:
        zones = update_data.get("available_zones") or product.get("available_zones")
        update_data["incoterms"] = clean_incoterms(update_data["incoterms"], zones)
