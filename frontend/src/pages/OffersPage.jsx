      ...
      <section className="pt-32 pb-8 px-5">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 tracking-tight">
            {i18n.t('offers.choisissez_votre_acces')} <span className="text-[#D9B35A]">{i18n.t('offers.a_la_centrale')}</span>
          </h1>
          <p className="text-white/70 text-lg">
            {i18n.t('offers.accedez_aux_prix_structurels')}
          </p>
        </div>
      </section>
            ...
            {subscriptionPlans.map((plan) => (
              ...
              >
                ...
                <ul className="m-0 p-0 list-none grid gap-2.5 relative mt-4">
                  {(i18n.t(`offers.features_${plan.id.replace(/-/g, '_')}`, { returnObjects: true }) || plan.features).map((feature) => (
                ...
                <div className="mt-auto pt-4 relative">
                  <Link to={`/inscription?plan=${plan.id}`}>
                    ...
                    >
                      {i18n.t('offers.s_inscrire_maintenant')}
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-6">
            <span className="badge-status mb-4 inline-flex">
              ...
              <span>{i18n.t('offers.wallet_credits')}</span>
            </span>
            <h2 className="text-2xl font-bold mt-3 mb-2">{i18n.t('offers.systeme_de_credits_o')}</h2>
            <p className="text-white/70 text-sm">{i18n.t('offers.complementaire_a_votre_abonnement')}</p>
          </div>
          
          <div className="glass-panel rounded-[22px] p-6">
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="font-semibold text-white/90 mb-4 text-sm uppercase tracking-wider">{i18n.t('offers.les_credits_financent')}</h3>
                <ul className="space-y-3">
                  {(i18n.t('offers.credits_usage', { returnObjects: true }) || []).map((item) => (
              ...
              <div className="glass-panel-soft rounded-2xl p-5">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <p className="text-sm text-white/70">
                    {i18n.t('offers.sans_lien_avec_le')} <strong className="text-white/90">{i18n.t('offers.n_impactent_jamais')}</strong>
                  </p>
                </div>
                
                <div className="border-t border-white/10 pt-4 mt-4">
                  <p className="text-xs text-white/50 mb-2">{i18n.t('offers.recharge_disponible')}</p>
                  <div className="flex flex-wrap gap-2">
                    {[50, 100, 200, 500].map((amount) => (
                      ...
                      >
                        {amount} {i18n.t('offers.credits_word')}
                      </span>
                    ))}
                  </div>
                </div>
      ...
      <section className="py-12 px-5">
        ...
        >
          <h2 className="text-2xl md:text-3xl font-bold mb-3">
            {i18n.t('offers.pret_a_rejoindre')}
          </h2>
          <p className="text-white/70 mb-6 text-lg">
            {i18n.t('offers.beneficiez_de_prix_structurels')}
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link to="/inscription">
              <button className="btn-gold inline-flex items-center justify-center gap-2 rounded-[14px] px-6 py-3.5 text-sm font-semibold">
                {i18n.t('offers.creer_mon_compte')}
                <ArrowRight className="w-4 h-4" />
              </button>
            </Link>
            <Link to="/#contact">
