      ...
      <div className="max-w-[1160px] mx-auto px-5 py-10">
        {error && (
          <div className="glass-panel-soft rounded-[18px] p-8 text-center" data-testid="logicoop-denied">
            ...
            <h1 className="text-xl font-bold text-white mb-2">{t('logicoop.denied_title', 'Espace réservé aux opérateurs LOGICOOP')}</h1>
        ...
        {op && (
          <div data-testid="logicoop-dashboard">
            <h1 className="text-2xl font-bold text-white mb-1">{t('logicoop.welcome', 'Bienvenue, {{name}}', { name: op.name })}</h1>
            <p className="text-sm text-white/55 mb-8">{t('logicoop.zones_intro', 'Vos zones opérationnelles assignées par la coopérative.')}</p>
            ...
            <div className="glass-panel-soft rounded-[18px] p-5 mb-4" data-testid="logicoop-missions">
              <h2 className="font-display text-lg text-white flex items-center gap-2 mb-1">
                <PackageCheck className="w-4 h-4 text-[#D9B35A]" /> {t('logicoop.missions_title', 'Missions en cours')} ({missions.length})
              </h2>
              <p className="text-xs text-white/45 mb-3">{t('logicoop.missions_desc', 'Commandes à enlever (EXW dans vos zones entrepôt) ou à livrer (CIF dans vos zones de livraison).')}</p>
                ...
                {missions.map((m) => (
                  <div key={m.order_id} className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-xs" data-testid={`logicoop-mission-${m.order_id}`}>
                    <span className={`px-2 py-0.5 rounded-lg text-[9px] font-bold ${m.mission === 'ENLEVEMENT' ? 'bg-[#D9B35A]/20 text-[#E9CF8E]' : 'bg-[#60A5FA]/20 text-[#60A5FA]'}`}>
                      {m.mission === 'ENLEVEMENT' ? t('logicoop.to_pickup', 'À ENLEVER') : t('logicoop.to_deliver', 'À LIVRER')}
                    </span>
                    ...
                    {m.logistics?.status === 'LIVREE' ? (
                      <span className="px-2 py-0.5 rounded-lg text-[9px] font-bold bg-emerald-500/15 text-emerald-400" data-testid={`mission-status-${m.order_id}`}>{t('logicoop.delivered', '✓ LIVRÉE')}</span>
                    ) : m.logistics?.status === 'PRISE_EN_CHARGE' ? (
                      <>
                        <span className="px-2 py-0.5 rounded-lg text-[9px] font-bold bg-[#D9B35A]/20 text-[#E9CF8E]" data-testid={`mission-status-${m.order_id}`}>{t('logicoop.in_progress', 'EN COURS')}</span>
                        <button type="button" onClick={() => setStatus(m, 'LIVREE')} data-testid={`mission-deliver-${m.order_id}`}
                          ...
                          {t('logicoop.mark_delivered', 'Marquer livrée')}
                        </button>
                      </>
                    ) : (
                      <button type="button" onClick={() => setStatus(m, 'PRISE_EN_CHARGE')} data-testid={`mission-take-${m.order_id}`}
                        ...
                        {t('logicoop.take_charge', 'Prendre en charge')}
                      </button>
                    )}
                  </div>
                ))}
                {!missions.length && <p className="text-sm text-white/45">{t('logicoop.no_missions', 'Aucune mission en cours dans vos zones.')}</p>}
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="glass-panel-soft rounded-[18px] p-5" data-testid="logicoop-exw-zones">
                <h2 className="font-display text-lg text-white flex items-center gap-2 mb-3">
                  <Warehouse className="w-4 h-4 text-[#D9B35A]" /> {t('logicoop.exw_zones', 'Zones entrepôt EXW')} ({op.exw_zones_detail.length})
                </h2>
                ...
                {!op.exw_zones_detail.length && <p className="text-sm text-white/45">{t('logicoop.no_exw', 'Aucune zone EXW assignée.')}</p>}
              </div>
              <div className="glass-panel-soft rounded-[18px] p-5" data-testid="logicoop-cif-zones">
                <h2 className="font-display text-lg text-white flex items-center gap-2 mb-3">
                  <Ship className="w-4 h-4 text-[#D9B35A]" /> {t('logicoop.cif_zones', 'Zones livraison CIF')} ({op.cif_zones_detail.length})
                </h2>
                ...
                {!op.cif_zones_detail.length && <p className="text-sm text-white/45">{t('logicoop.no_cif', 'Aucune zone CIF assignée.')}</p>}
              </div>
            </div>
          </div>
        )}
