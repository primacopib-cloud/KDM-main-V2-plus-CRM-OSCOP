...
export const detaillantAPI = {
  ...
  weeklyRecaps: () => apiCall('/detaillant/weekly-recaps'),
  convention: () => apiCall('/detaillant/convention'),
  signConvention: (signerName) => apiCall('/detaillant/convention/sign', { method: 'POST', body: JSON.stringify({ signer_name: signerName }) }),
  cession: (offerId) => apiCall(`/detaillant/offers/${offerId}/cession`),
  signCession: (offerId, payload) => apiCall(`/detaillant/offers/${offerId}/cession/sign`, { method: 'POST', body: JSON.stringify(payload) }),
