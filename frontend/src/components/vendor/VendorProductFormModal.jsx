...
import { Package, Plus, RefreshCw, TrendingUp, Save } from 'lucide-react';
...
import { CATEGORIES, TVA_RATES } from './vendorConstants';
import { ProductPhotoUploader, uploadProductPhotos } from './ProductPhotoUploader';
import { ZonesIncotermsSection } from './ZonesIncotermsSection';
import { ProductLogisticsSections } from './ProductLogisticsSections';
...
export const VendorProductFormModal = ({ isOpen, onClose, onSuccess, vendorId, countries, editProduct = null }) => {
  ...
  const handleZoneToggle = (zone) => {
    ...
    setFormData(prev => {
      const incoterms = { ...(prev.incoterms || {}) };
      if (selected) delete incoterms[zone];
      return {
        ...prev,
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ? 'Champs modifiables : nom, description, prix HT, stock, quantité min., zones, incoterms et DLC. Les autres champs sont figés après soumission.'
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Stock / Origine / Conservation */}
          <ProductLogisticsSections formData={formData} handleChange={handleChange} countries={countries} />

          {/* Zones + Incoterms */}
          <ZonesIncotermsSection
            formData={formData}
            allowance={allowance}
