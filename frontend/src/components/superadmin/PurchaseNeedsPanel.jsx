...
const STATUS = { NEW: ['Nouveau', 'text-amber-300 bg-amber-500/10 border-amber-400/40'], ASSIGNED: ['Assigné', 'text-[#8CC63E] bg-[#8CC63E]/10 border-[#8CC63E]/30'] };
            ...
            return (
              <div key={n.id} className="rounded-xl px-3 py-2.5 bg-white/[0.02] border border-white/[0.06] text-[11px] text-white/70" data-testid={`need-row-${n.id}`}>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-white font-semibold">{n.product}</span>
                  ...
                  {n.communityplace && <span className="px-2 py-0.5 rounded-full border font-semibold text-sky-300 bg-sky-500/10 border-sky-400/40">CommunityPlace</span>}
                  <span className="ml-auto flex gap-1.5">
                    ...
                    {!n.communityplace && (
                      <button type="button" data-testid={`need-community-${n.id}`}
                        onClick={() => post(`${API_URL}/api/admin/purchase-needs/${n.id}/communityplace`, null, 'Publié sur la CommunityPlace payante')}
