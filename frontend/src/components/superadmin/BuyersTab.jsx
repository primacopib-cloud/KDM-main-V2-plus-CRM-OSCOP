    ...
    <div className="space-y-6" data-testid="buyers-tab">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm opacity-60 mt-1">Comptes acheteurs B2B : crédits, commandes, suspension. Organisations complètes via Admin Orgs.</p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="h-10 pl-9 pr-3 rounded-lg bg-white/60 border border-black/10 text-sm text-[#1F2A3A] w-64"
          />
        </div>
      </div>
      ...
      <div className="glass-panel-soft rounded-[18px] p-5 overflow-x-auto" data-testid="buyers-list">
        <table className="w-full text-sm">
          ...
          <tbody>
            {buyers.map((b) => (
              <tr key={b.id} className={`border-b border-black/5 ${b.suspended ? 'opacity-50' : ''}`} data-testid={`buyer-row-${b.id}`}>
                <td className="py-2.5 pr-3">
                  <p className="font-medium text-[#1F2A3A]">{b.contact_name || '—'}</p>
                  <p className="text-xs opacity-50">{b.email}</p>
                </td>
                <td className="py-2.5 pr-3 text-[#1F2A3A]">{b.company_name || '—'}</td>
                <td className="py-2.5 pr-3 text-right">{b.orders_count}</td>
                <td className="py-2.5 pr-3 text-right">
                  <input
                    ...
                    className="h-8 w-24 px-2 rounded-lg bg-white/60 border border-black/10 text-sm text-right text-[#1F2A3A]"
                  />
                </td>
                <td className="py-2.5 text-right whitespace-nowrap">
                  ...
                  <button type="button" onClick={() => toggleSuspend(b)} data-testid={`buyer-suspend-${b.id}`}
                    ...
                    className={`p-1.5 rounded-lg opacity-50 hover:opacity-100 transition-all ${b.suspended ? 'text-emerald-600 hover:bg-emerald-500/10' : 'text-red-500 hover:bg-red-500/10'}`}>
        ...
        {buyers.length === 0 && <p className="text-sm opacity-50 text-center py-6">Aucun acheteur trouvé.</p>}
      </div>
    </div>
  );
};
