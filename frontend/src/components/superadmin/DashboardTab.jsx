    ...
    <div className="grid lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <KPISection title={i18n.t('adm.utilisateurs_organisations')} icon={Users} color="#3B82F6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
              ...
              <div className="text-xs text-white/50">{i18n.t('adm.total_utilisateurs')}</div>
            </div>
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
              <div className="text-xl font-bold text-green-400">{kpis?.users?.active}</div>
              <div className="text-xs text-white/50">{i18n.t('adm.actifs')}</div>
            </div>
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
              <div className="text-xl font-bold text-white">{kpis?.users?.new_period}</div>
              <div className="text-xs text-white/50">{i18n.t('adm.nouveaux_period', { period })}</div>
            </div>
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
              <div className="text-xl font-bold text-[#8B5CF6]">{kpis?.users?.organizations?.approved}</div>
              <div className="text-xs text-white/50">{i18n.t('adm.orgs_approuvees')}</div>
            </div>
          </div>

          {kpis?.users?.by_role && Object.keys(kpis.users.by_role).length > 0 && (
            <div className="mt-4 pt-4 border-t border-white/5">
              <p className="text-xs text-white/50 mb-2">{i18n.t('adm.par_role')}</p>
              <div className="flex flex-wrap gap-2">
        ...
        <KPISection title={i18n.t('adm.catalogue_produits_2')} icon={Package} color="#F59E0B">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
              ...
              <div className="text-xs text-white/50">{i18n.t('adm.total_produits')}</div>
            </div>
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
            ...
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
              ...
              <div className="text-xs text-white/50">{i18n.t('adm.stock_faible')}</div>
            </div>
            <div className="text-center p-3 rounded-xl bg-white/[0.02]">
              <div className="text-xl font-bold text-red-400">{kpis?.products?.out_of_stock}</div>
              <div className="text-xs text-white/50">{i18n.t('adm.rupture')}</div>
            </div>
          </div>
        </KPISection>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <KPISection title={i18n.t('adm.signatures_eidas')} icon={FileSignature} color="#8B5CF6">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-white/60">{i18n.t('adm.total_signatures')}</span>
                <span className="font-bold text-[#8B5CF6]">{kpis?.signatures?.total}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-white/60">{i18n.t('adm.signees')}</span>
                <span className="font-semibold text-green-400">{kpis?.signatures?.signed}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-white/60">{i18n.t('adm.en_attente')}</span>
                <span className="font-semibold text-yellow-400">{kpis?.signatures?.pending}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-white/60">{i18n.t('adm.taux_de_succes')}</span>
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden">
          <div className="px-5 py-3 flex items-center justify-between bg-red-500/5 border-b border-red-500/10">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-sm text-red-400">{i18n.t('adm.alertes')}</h3>
            </div>
            <span className="text-xs text-red-400/60">{i18n.t('adm.alertes_count', { count: alerts.length })}</span>
          </div>
        ...
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden">
          <div className="px-5 py-3 flex items-center justify-between bg-white/[0.02] border-b border-white/[0.05]">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-sm text-white/80">{i18n.t('adm.activite_recente')}</h3>
            </div>
          </div>
        ...
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] p-5">
          <h3 className="font-semibold text-sm text-white/80 mb-4">{i18n.t('adm.actions_rapides')}</h3>
          <div className="space-y-2">
            <Link to="/admin-v2" className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] transition-colors">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/80">{i18n.t('adm.gerer_les_organisations')}</span>
              </div>
            ...
            <Link to="/catalogue" className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] transition-colors">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/80">{i18n.t('adm.catalogue_produits')}</span>
              </div>
              <ChevronRight className="w-4 h-4 text-white/40" />
            </Link>
            <Link to="/legal/charte-ess" className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] transition-colors">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/80">{i18n.t('adm.charte_ess')}</span>
              </div>
              <ChevronRight className="w-4 h-4 text-white/40" />
            </Link>
            <button
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/80">{i18n.t('adm.gerer_le_catalogue')}</span>
              </div>
              <ChevronRight className="w-4 h-4 text-white/40" />
            </button>
          </div>
