...
const inputCls = 'h-10 px-3 rounded-lg bg-white/60 border border-black/10 text-sm text-[#1F2A3A] w-full';
...
export const GrantRoleForm = ({ roles, onDone }) => {
  ...
  return (
    <div className="glass-panel-soft rounded-[18px] p-5" data-testid="team-grant-form">
      <h3 className="font-display text-lg mb-3 text-[#1F2A3A]">{i18n.t('adm.team_grant_title')}</h3>
      <div className="relative mb-3">
        ...
        {results.length > 0 && !selected && (
          <div className="absolute z-10 left-0 right-0 mt-1 rounded-lg bg-white shadow-xl border border-black/10 max-h-52 overflow-y-auto">
            {results.map((u) => (
              <button
                key={u.id} type="button"
                onClick={() => { setSelected(u); setResults([]); }}
                data-testid={`team-search-result-${u.id}`}
                className="w-full text-left px-3 py-2 text-sm hover:bg-[#D9B35A]/10 text-[#1F2A3A]"
              >
...
export const CreateMemberForm = ({ roles, onDone }) => {
  ...
  return (
    <div className="glass-panel-soft rounded-[18px] p-5" data-testid="team-create-form">
      <h3 className="font-display text-lg mb-3 text-[#1F2A3A]">{i18n.t('adm.team_create_title')}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" data-testid="team-created-modal">
          <div className="rounded-[20px] p-6 max-w-md w-full bg-white" style={{ boxShadow: '0 24px 64px rgba(76,42,110,0.3)' }}>
            <div className="flex items-start justify-between mb-2">
              <h4 className="font-display text-lg text-[#1F2A3A] flex items-center gap-2">
            ...
            <p className="text-sm text-[#1F2A3A] mb-1">{created.user.contact_name} — {created.user.email}</p>
            <p className="text-xs opacity-60 mb-3">{i18n.t('adm.team_temp_password')} :</p>
            <div className="flex items-center gap-2 mb-3">
              <code data-testid="team-temp-password" className="flex-1 px-3 py-2 rounded-lg bg-black/5 text-sm font-mono text-[#1F2A3A]">{created.temp_password}</code>
