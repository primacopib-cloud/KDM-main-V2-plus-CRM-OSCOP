  ...
  return (
    <CollapsePanel icon={ShoppingCart} title="Paniers abandonnés (réservations expirées sans commande)" testid="abandoned-carts-panel">
      {() => {
        ...
        return (
          <>
            <p className="text-[11px] text-white/50 mb-2">{data.total} réservation(s) expirée(s) au total</p>
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-white/40">
                  ...
                  <th className="py-1.5 pr-2 text-center">Prolong.</th><th className="py-1.5">Expirée le</th>
                </tr>
              </thead>
              <tbody>
                {data.entries.map((e) => (
                  <tr key={e.id} className="border-t border-white/[0.06] text-xs" data-testid="abandoned-row">
                    ...
                    <td className="py-2 text-white/50 font-mono">{fmtDate(e.expired_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
